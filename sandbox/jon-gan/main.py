...
class Discriminator(nn.Module):
    def __init__(self, in_channels=2, num_channels=32):
        ...
        self.fc = nn.Linear(3*num_channels, 1)
...
if __name__ == "__main__":
    ...
    lissajous = lissajous_walk(z_dim, num_points=5)
        ...
        if i % 1000 == 0:
            ...
            for j, z_test in tqdm(enumerate(lissajous)):
                ...
                c = c.cpu().detach().numpy().transpose(0, 2, 1).reshape(
                    -1, 3, 2).transpose(0, 2, 1).astype(np.float64)
                raster([[a, b, c]])
                plt.savefig("outs15/test{:010d}_{:05d}.png".format(i, j))
                plt.close()
            gen.train()
            ...
            torch.save(checkpoint, "outs15/checkpoint{:010d}.pt".format(i))
