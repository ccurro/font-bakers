...
from data import CHARACTER_INDICES
...
class StyleBlock(nn.Module):
    ...
    def forward(self, style, x=None):
        ...
        return AdaIN(F.relu(a), gains, biases)
...
class MappingNet(nn.Module):
    def __init__(self, z_dim, width, num_layers):
        ...
        self.embed = nn.Embedding(len(CHARACTER_INDICES), width)
...
class StyleNet(nn.Module):
    def __init__(self, num_curves, num_blocks, style_dim, z_dim, num_channels):
        ...
        self.mapping = MappingNet(z_dim, style_dim, 2)
        self.styleblock1 = StyleBlock(
            num_channels,
            style_dim=2*style_dim,
            init_width=init_width,
            initial=True)
        self.styleblock2 = StyleBlock(num_channels, style_dim=2*style_dim)
        self.styleblocks = nn.ModuleList([
            StyleBlock(num_channels, style_dim=2*style_dim)
